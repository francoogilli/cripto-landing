            ...
            <div className="divide-y divide-zinc-800">
                {QuestionsData.map(({ question, answer }, index) => (
                    <div className={`accordion-item text-white duration-300 rounded-lg ${openItems[index] ? 'bg-[#131313]' : 'hover:bg-[#131313]'}`} key={index}>
                        <div className="flex justify-between px-4 py-5 text-base md:text-lg  font-semibold cursor-pointer" onClick={() => toggleAccordion(index)}>
                            {question}
                            <svg xmlns="http://www.w3.org/2000/svg" className=" bg-[#7F35FF] rounded-full p-1" width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
